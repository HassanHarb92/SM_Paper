...
def main():
    ...
    for run_idx in range(1, args.runs + 1):
        ...
        pd.DataFrame(per_q_rows).to_csv(out_dir / f"train_run_{run_idx:02d}_predictions.csv", index=False)
    ...
    pd.DataFrame([{"Runs": args.runs, "Mean Accuracy": mean_acc, "SD Accuracy": sd_acc,
                 ...
                 ).to_csv(out_dir / "train_runs_aggregate.csv", index=False)
...
Artifacts:
- train_run_XX_predictions.csv
- train_runs_summary.csv  (Wilson & Exact 95% CIs over items)
- train_runs_aggregate.csv  (mean ± SD and a 95% CI across runs)
- train_runs_all_rows.csv
- logs/train_run_XX.txt
"""
