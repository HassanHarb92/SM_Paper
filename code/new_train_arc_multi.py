#!/usr/bin/env python3
"""
Train-only multi-run evaluator for the SM agent on ARC-Challenge-Train.

- Reads ONE CSV:  ~/Desktop/Codes/SM_Paper/ai2_benchmark/ARC-Challenge-Train.csv
...
- Never overwrites old results: creates /Users/hharb/Desktop/Codes/SM_Paper/ARC-train-multi/<timestamp>/
...
def main():
    parser = argparse.ArgumentParser(description="Train-only multi-run evaluator on ARC-Challenge-Train.")
    parser.add_argument("--csv_path", type=str,
                        default=str(Path.home() / "Desktop/Codes/SM_Paper/ai2_benchmark/ARC-Challenge-Train.csv"),
                        help="Path to ARC-Challenge-Train.csv")
    parser.add_argument("--out_root", type=str,
                        default="/Users/hharb/Desktop/Codes/SM_Paper/ARC-train-multi",
    ...
    if not csv_path.exists():
        raise FileNotFoundError(f"ARC Train CSV not found: {csv_path}")
    ...
    out_dir = out_root / f"train_multi_{stamp}"
    ...
    for run_idx in range(1, args.runs + 1):
        ...
        correct = 0
        per_q_rows = []
        log_path = logs_dir / f"train_run_{run_idx:02d}.txt"
        ...
        per_run_pred_path = out_dir / f"train_run_{run_idx:02d}_predictions.csv"
    ...
    per_run_df.to_csv(out_dir / "train_runs_summary.csv", index=False)
    ...
    agg_df.to_csv(out_dir / "train_runs_aggregate.csv", index=False)

    # Save all rows together
    pd.DataFrame(all_rows).to_csv(out_dir / "train_runs_all_rows.csv", index=False)

    # README with configuration
    readme = f"""Train multi-run evaluation on ARC-Challenge-Train
...
Artifacts:
- Per-run predictions: train_run_XX_predictions.csv
- Per-run summaries:   train_runs_summary.csv  (includes Wilson & Exact 95% CIs over items)
- Across-run aggregate: train_runs_aggregate.csv  (mean ± SD and a 95% CI across runs)
- Combined rows:       train_runs_all_rows.csv
- Raw outputs:         logs/train_run_XX.txt
...
"""
    ...
    print(f"📄 Per-run summary:   {out_dir / 'train_runs_summary.csv'}")
    print(f"📄 Across-run aggregate: {out_dir / 'train_runs_aggregate.csv'}")
