...
def main():
    ...
    parser.add_argument("--csv_path", type=str,
                        ...
                        help="Path to ARC-Challenge-Test.csv")
    ...
    for run_idx in range(1, args.runs + 1):
        ...
        per_run_pred_path = out_dir / f"test_run_{run_idx:02d}_predictions.csv"
    ...
    agg_df.to_csv(out_dir / "test_runs_aggregate.csv", index=False)
...
Artifacts:
- Per-run predictions: test_run_XX_predictions.csv
- Per-run summaries:   test_runs_summary.csv  (includes Wilson & Exact 95% CIs over items)
- Across-run aggregate: test_runs_aggregate.csv  (mean ± SD and a 95% CI across runs)
- Combined rows:       test_runs_all_rows.csv
- Raw outputs:         logs/test_run_XX.txt
...
"""
    ...
    print(f"📄 Per-run summary:   {out_dir / 'test_runs_summary.csv'}")
    print(f"📄 Across-run aggregate: {out_dir / 'test_runs_aggregate.csv'}")
